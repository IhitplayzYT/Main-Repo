...
void add(Node_h **hashtable, void *val, int flag) {
  if (flag == 0) {
...
bool in_set(Node_h **arr, void *data) {
  for (int i = 0; i < arr[0]->len; i++) {
    Node_h *ptr = arr[i];
    while (ptr != NULL) {
      if (ptr->flag == 0 && *((int *)ptr->data) == *((int *)data)) {
        return true;
      } else if (ptr->flag == 0 && !strcmp((char *)data, (char *)ptr->data)) {
        return true;
      }
      ptr = ptr->next;
    }
