...
public i16 floor_div(i16 a, i16 b) { return (i16)a / b; }

public i8 _getbit(i8 *a, i16 n) { return (((1 << n) & *a) >> n); }
public void _setbit(i8 *a, i16 n) { *a |= (1 << n); }
public void _unsetbit(i8 *a, i16 n) { *a &= (~(1 << n)); }
public void _flipbit(i8 *a, i16 n) { *a ^= (1 << n); }
...
public i16 net_port(i16 x) { return endian16(x); }
...
public i64 seconds_elapsed() { return ticks_elapsed() / tick_freq(); }
...
public Time *curr_time() {
  ...
  ts->days %= 365;
  ts->date_of_month = (ts->days <= 31)    ? ts->days
                      : (ts->days <= 59)  ? ts->days - 31
                      : (ts->days <= 90)  ? ts->days - 59
                      : (ts->days <= 120) ? ts->days - 90
                      : (ts->days <= 151) ? ts->days - 120
                      : (ts->days <= 181) ? 6
                      : (ts->days <= 212) ? ts->days - 181
                      : (ts->days <= 243) ? ts->days - 212
                      : (ts->days <= 273) ? ts->days - 243
                      : (ts->days <= 304) ? ts->days - 273
                      : (ts->days <= 334) ? ts->days - 304
                      : (ts->days <= 365) ? ts->days - 334
                                          : 0;
  return ts;
}
...
public i8 *fmttime(Time *ts) {
  ...
  if (!buff)
    return (i8 *)0;
  snprintf(buff, 49, "%s, %.02hu:%.02hu:%.02hu , %.02hu %s,%.04hu",
           (ts->weekday == 0)   ? "Monday"
           : (ts->weekday == 1) ? "Tuesday"
           : (ts->weekday == 2) ? "Wednesday"
           : (ts->weekday == 3) ? "Thursday"
           : (ts->weekday == 4) ? "Friday"
           : (ts->weekday == 5) ? "Saturday"
           : (ts->weekday == 6) ? "Sunday"
                                : "INVALID",
           ts->hours, ts->minutes, ts->seconds, ts->date_of_month,
           (ts->month == 1)    ? "January"
           : (ts->month == 2)  ? "February"
           : (ts->month == 3)  ? "March"
           : (ts->month == 4)  ? "April"
           ...
           : (ts->month == 7)  ? "July"
           : (ts->month == 8)  ? "August"
           : (ts->month == 9)  ? "September"
           : (ts->month == 10) ? "October"
           : (ts->month == 11) ? "November"
           : (ts->month == 12) ? "December"
                               : "INVALID",
           ts->years);
  return buff;
}
...
public i8 flip_byte(i8 ch) { return ~ch; }
